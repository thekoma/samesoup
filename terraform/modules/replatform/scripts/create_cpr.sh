...
readonly CPR_RESOURCE=cloudruns.operator.run.cloud.google.com
for _i in {1..18}; do
  echo "Ensuring CloudRun exists in cluster... attempt ${_i}"
...
kubectl wait --for=condition=Ready cloudruns.operator.run.cloud.google.com ${REVISION_NAME} --timeout 10m